...
class BattleDisplay extends React.Component {
  ...
  attackDamageCalculation = playerAttack => {
    ...
    if (isPlayerWin) {
      this.setState({
        monsterRemainingHp: Math.max(
          this.state.monsterRemainingHp - playerdata[0].base.Attack,
          0
        ),
      ...
      this.setState({
        playerRemainingHp: Math.max(
          this.state.playerRemainingHp -
            monsterdata[this.state.monsterID].base.Attack,
          0
        ),
        triggerDialogueBox: true,
        triggerTieDialogueBox: false,
    ...
    return (
      <div>
        <div className="display">
            ...
            {isMonsterAlive && isPlayerAlive && (
              <div>
                <button
                  ...
                  onClick={() => this.attackDamageCalculation("slash")}
                >
                  Slash
                </button>
                <button
                  className="shield-button"
                  onClick={() => this.attackDamageCalculation("shield")}
                >
                  Shield
                </button>
                <button
                  className="stab-button"
                  onClick={() => this.attackDamageCalculation("stab")}
                >
                  Stab
                </button>
              </div>
          ...
          <div>
            <MonsterCard
              monster={monsterdata[this.state.monsterID]}
              HP={this.state.monsterRemainingHp}
            />
          </div>
        </div>
        ...
        {isPlayerAlive &&
          this.state.triggerTieDialogueBox &&
          isMonsterAlive && (
            <TieDialogueBox playerAction={this.state.playerAction} />
          )}
